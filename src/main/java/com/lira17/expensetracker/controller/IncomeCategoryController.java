...
public class IncomeCategoryController {
    ...
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Income category is updated",
                    content = {@Content(mediaType = "application/json", schema = @Schema(implementation = IncomeCategoryGetDto.class))}),
            @ApiResponse(responseCode = "404", description = "Income category is not found", content = @Content)})
    public IncomeCategoryGetDto updateCategory(@PathVariable("id") Long id, @RequestBody IncomeCategoryCreateDto incomeCategoryCreateDto) {
        var category = incomeCategoryMapper.convertToCategory(incomeCategoryCreateDto);
        return incomeCategoryMapper.convertToDto(incomeCategoryService.updateCategory(id, category));
    }
